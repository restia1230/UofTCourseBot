...
function field(items){
    ...
    for(var i = 0; i< items.length; i++){
      var item = {  
        name:  `${items[i].name}`,
        value: `Description: ${replace1(items[i].description)}` +"\n"+
...
exports.run = async (client, message, args, ops) => {
    ...
    let food = await fetch(`https://nikel.ml/api/food?tags=${query}&limit=5`)
        .then(res => res.json()).catch(err =>{
            return message.channel.send("Food not found.")});
    // console.log(food)
    // var v = await JSON.parse(food);
    // console.log(v)
    if(food.status_message =="success: results not found"){
        return message.channel.send("Food not found.");
    }

    var arr = field(food.response);
    message.channel.send({embed: {
        ...
        title: `Food search result for ${query}`,
        fields: arr,
        timestamp: new Date(),
        footer: {
          icon_url: profilepic ,
